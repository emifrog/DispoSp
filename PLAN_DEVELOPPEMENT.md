...
État au 22 septembre 2026. **Les dix-neuf migrations sont appliquées** sur le projet de développement, les correctifs B1 à B6 et C1 à C16 du 22 septembre compris ; le tableau des migrations du [README](README.md) fait foi. Le reste tient à la configuration de l’hébergement et à la recette, pas au schéma. Les statuts ci-dessous distinguent les fonctionnalités implémentées des vérifications restant à effectuer sur le site hébergé.
...
| Base de données             | ✅ implémentée         | 22 tables, dix-neuf migrations appliquées, isolation RLS et règles métier                                                            |
...
Versionnement et outillage, authentification, lecture des données, intégration continue et virtualisation de la synthèse. Les dix-neuf migrations sont appliquées ; le schéma compte 22 tables.
...
| Modèle de données (§14)                        | ✅   | 22 tables ; dix-neuf migrations appliquées                                                         |
...
| Emails en attente ou renvoyés                       | Agents non prévenus ou messages en double           | File avec bail, tentatives et état (`20260922150000`, appliquée), un message à la fois           |
